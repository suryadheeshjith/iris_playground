...
class ActorCritic(nn.Module):
    def __init__(self, act_vocab_size, use_original_obs: bool = False, is_ram: bool = False) -> None:
        super().__init__()
    ...
    def compute_bc_loss(self, batch: Batch) -> LossWithIntermediateLosses:
        ...
        # actor_loss = F.cross_entropy(outputs_ac.logits_actions[:,0], expert_actions.squeeze())
    ...
    def trajectory(self, test_env, max_steps=1000):
        ...
        while not done and steps < max_steps:
            obs_list.append(obs)
            obs = torch.FloatTensor(obs).div(255).cuda()
            obs = rearrange(obs, 'n h w c -> n c h w')
            outputs_ac = self(obs)
            action_token = Categorical(logits=outputs_ac.logits_actions).sample() # outputs_ac.logits_actions.argmax(dim=-1)
            obs, reward, done, _ = test_env.step(action_token)
            episode_env_reward += reward
            episode_length += 1
            steps += 1
            
        return episode_env_reward, episode_length, obs_list
    ...
    def imagine(self, batch: Batch, tokenizer: Tokenizer, world_model: WorldModel, horizon: int, show_pbar: bool = False) -> ImagineOutput:
        ...
        wm_env = WorldModelEnv(tokenizer, world_model, device, is_ram=self.is_ram)
